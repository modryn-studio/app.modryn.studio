...
function CodeBlock({ children, className, ...props }: CodeBlockProps) {
  return (
    <div
      className={cn(
        'not-prose flex w-full flex-col overflow-clip rounded-sm border',
        'border-panel-border bg-panel-input text-panel-text',
        className
      )}
      {...props}
    >
...
function CodeBlockCode({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        // Unsupported language — fall back to plain pre/code
  ...
  return highlightedHtml ? (
    // Content comes from shiki (trusted syntax highlighter) — not user input
