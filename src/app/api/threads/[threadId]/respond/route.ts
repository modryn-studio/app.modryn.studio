...
// Injected into system prompt for thread contributions.
// Each member sees all prior responses — they should build on or challenge what was said.
const THREAD_FORMAT =
  'You are contributing to a group strategy thread. Your response should be 150–300 words. Reference at least one prior contribution by name — build on it or challenge it directly. Lead with your sharpest point. Do not summarize what others said before adding your view.';
...
export async function POST(
  ...
  try {
    ...
    const systemPrompt = assembleContext([
      { label: 'format', content: `## Format Instructions\n\n${THREAD_FORMAT}`, priority: 1 },
      { label: 'system', content: member.system_prompt, priority: 2 },
      ...(companyContext
